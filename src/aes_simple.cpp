...
void AddRoundKey(unsigned char state[][4], uint w[])
{
...
uint SubWord(unsigned int word)
{
   unsigned int result;
...
// constructor
AES_SIMPLE::AES_SIMPLE(uchar* buf_in, uchar* buf_out, uint buf_len, uchar* key, string aes_method) :
	AES_BASE(buf_in, buf_out, buf_len, key, aes_method){}

// do simple encryption - no special hw
void AES_SIMPLE::encrypt()
{
	unsigned int key_schedule[60];
	KeyExpansion(key, key_schedule, 128);

	for(uint i=0; i<buf_len; i+=16)
		aes_encrypt(buf_in, buf_out, key_schedule, 128);
}

// do simple decryption - no special hw
void AES_SIMPLE::decrypt()
{ 
	unsigned int key_schedule[60];
	KeyExpansion(key, key_schedule, 128);

	for(uint i=0; i<buf_len; i+=16)
		aes_decrypt(buf_in, buf_out, key_schedule, 128);
}
