// Original code from Adrian Belgun bachelor
// Version 0.1 Lupescu Grigore 
...
// AES operates on a 4\D74 column-major order matrix of bytes, termed the state.
/*
...
{
	return (uchar4)(
		// 2 * state.s0 + 3 * state.s1 +     state.s2 +     state.s3
		AES_Galois_2[state.s0] ^ AES_Galois_3[state.s1] ^state.s2  ^state.s3,
		//     state.s0 + 2 * state.s1 + 3 * state.s2 +     state.s3
		state.s0  ^ AES_Galois_2[state.s1] ^ AES_Galois_3[state.s2] ^state.s3,
		//     state.s0 +     state.s1 + 2 * state.s2 + 3 * state.s3
		state.s0 ^state.s1 ^ AES_Galois_2[state.s2] ^ AES_Galois_3[state.s3],
		// 3 * state.s0 +     state.s1 +     state.s2 + 2 * state.s3
		AES_Galois_3[state.s0] ^ state.s1 ^ state.s2  ^ AES_Galois_2[state.s3]
		);
}
...
{
	return (uchar4)(
		AES_Galois_E[state.s0] ^ AES_Galois_B[state.s1] ^
		AES_Galois_D[state.s2] ^ AES_Galois_9[state.s3],
		AES_Galois_9[state.s0] ^ AES_Galois_E[state.s1] ^
		AES_Galois_B[state.s2] ^ AES_Galois_D[state.s3],
		AES_Galois_D[state.s0] ^ AES_Galois_9[state.s1] ^
		AES_Galois_E[state.s2] ^ AES_Galois_B[state.s3],
		AES_Galois_B[state.s0] ^ AES_Galois_D[state.s1] ^
		AES_Galois_9[state.s2] ^ AES_Galois_E[state.s3]
		);
}
